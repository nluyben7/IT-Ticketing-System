...
def init_db():
    ...
    db.execute(''' CREATE TABLE IF NOT EXISTS tickets (
            ...
            priority TEXT,
            affects_others INTEGER NOT NULL DEFAULT 0,
...
def seed_db():
    ...
    for ticket in seeded_tickets:
        db.execute('''INSERT OR REPLACE INTO tickets (ticketer_name, ticketer_email, issue_type, priority, affects_others, summary, description, status, resolution_details, specialist_username_assigned, submission_date, assignment_date, resolution_date)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
            ticket["ticketer_name"],
            ticket["ticketer_email"],
            ticket["issue_type"],
            ticket["priority"],
            int(ticket["affects_others"]),
            ticket["summary"],
            ticket["description"],
            ticket["status"],
